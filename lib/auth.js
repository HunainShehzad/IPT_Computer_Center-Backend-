...
async function validateSessionToken(token) {
  // If no sessionToken in the JWT at all, the JWT itself is valid proof
  // (older tokens or tokens from the admin hardcoded path that missed the
  // sessionToken embed) — allow through so we don't lock everyone out.
  if (!token?.sessionToken) return { _noSession: true };
  ...
  try {
    ...
    if (!session) {
      // Admin sessions may not persist across backend restarts.
      // If the JWT role is admin and the token decoded correctly (secret
      // matched), trust the JWT itself — the DB session is advisory only.
      if (token.role === "admin") return { _adminFallback: true };
      return null;
    }

    if (session.status !== "active") {
      // Same admin fallback — admin is never displaced
      if (token.role === "admin") return { _adminFallback: true };
      return null;
    }
